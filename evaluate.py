...
def parse_args():
    ...
    parser.add_argument(
        "--attack_method", type=str, default="pgd", choices=['pgd', 'fgsm'], help="Attack method(PGD or FGSM)"
    )
    parser.add_argument(
...
def main():
    ...
    if args.attack_method == "fgsm":
        attacker = attack_util.PGDAttack(
            attack_step=1, eps=eps, alpha=alpha, loss_type="ce",
            targeted=args.targeted, attack_target=target_label, num_classes=num_classes)
    else:
        attacker = attack_util.PGDAttack(
